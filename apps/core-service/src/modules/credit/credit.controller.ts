...
export class CreditController {
  ...
  @Get('limit')
  @ApiOperation({ summary: 'Get credit limit' })
  async getCreditLimit(@Request() req): Promise<{ creditLimit: number }> {
    const account = await this.creditService.getCreditAccount(req.user.id);
    return { creditLimit: account.creditLimit };
  }
}
