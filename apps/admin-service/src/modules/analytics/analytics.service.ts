  ...
  async getDashboardStats() {
    ...
    const loanStatsResult = await this.loanRepository
      ...
      .where('loan.status IN (:...statuses)', {
        statuses: [ELoanStatus.DISBURSED, ELoanStatus.ACTIVE, ELoanStatus.DEFAULTED],
      })
      .getRawOne();
    ...
    const recentTransactionsResult = await this.transactionRepository
      ...
      .where('transaction.status = :status', { status: ETransactionStatus.COMPLETED })
      .andWhere('transaction.created_at >= :startDate', { startDate: thirtyDaysAgo })
      .getRawOne();
    ...
    const creditStatsResult = await this.creditAccountRepository
      ...
      .addSelect('SUM(credit_account.outstanding_balance)', 'totalOutstandingBalance')
      .getRawOne();
    ...
    return {
      ...
      credit: {
        totalCreditLimit: parseFloat(creditStatsResult?.totalCreditLimit || '0'),
        totalAvailableCredit: parseFloat(creditStatsResult?.totalAvailableCredit || '0'),
        totalOutstandingBalance: parseFloat(creditStatsResult?.totalOutstandingBalance || '0'),
        utilizationRate:
