...
import { NOTIFICATION_GRPC_PACKAGE } from './notification-grpc.module';
...
interface NotificationGrpcServiceInterface {
  sendEmail(data: SendEmailRequest): any;
}
...
export class NotificationGrpcService implements OnModuleInit {
  ...
  async sendEmail(
    ...
    try {
      ...
      this.logger.error(
        `gRPC sendEmail failed: ${error.message}`,
        error.stack,
      );
      // Return error response instead of throwing to allow admin action to proceed
      return {
        success: false,
